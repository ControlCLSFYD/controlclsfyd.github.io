...
interface CourtGameProps extends BaseGameProps {}

const CourtGame: React.FC<CourtGameProps> = ({ onGameComplete, onPlayAgain, difficulty = 1 }) => {
  ...
  const [secretPhraseProgress, setSecretPhraseProgress] = useState(0);
  const secretPhrase = "i can handle the truth";
  ...
  // Handle key presses for secret phrase
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      const expectedChar = secretPhrase[secretPhraseProgress];
      if (e.key.toLowerCase() === expectedChar) {
        const newProgress = secretPhraseProgress + 1;
        setSecretPhraseProgress(newProgress);
        
        if (newProgress === secretPhrase.length) {
  ...
  }, [secretPhraseProgress, secretPhrase]);
  ...
  return (
    <div className="flex flex-col items-center justify-center mt-4">
      <h2 className="text-xl mb-4">COURT CHALLENGE</h2>
        ...
        {showInstructions ? (
          ...
          <div>
            ...
            {secretPhraseProgress > 0 && secretPhraseProgress < secretPhrase.length && (
              <p className="text-xs text-terminal-green">Secret phrase progress: {(secretPhraseProgress / secretPhrase.length * 100).toFixed(0)}%</p>
            )}
          </div>
        )}
      </div>
