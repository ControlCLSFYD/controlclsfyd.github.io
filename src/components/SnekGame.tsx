...
const SECRET_PHRASE = "thank you snek";

const SnekGame: React.FC<BaseGameProps> = (props) => {
      ...
      if (key.length === 1) { // Only process single characters
        ...
        if (SECRET_PHRASE.startsWith(newBuffer)) {
          ...
          if (newBuffer === SECRET_PHRASE) {
            console.log("Secret phrase completed: Snek victory!");
            setGameWon(true);
  ...
  // Pass the original SnakeGame props along with a forced win if secret is activated
  return (
    <SnakeGame 
      {...props} 
      forceWin={gameWon}
